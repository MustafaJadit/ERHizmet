...
//		setBackgroundResource(R.drawable.wheel_bg);
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (maxLength > 0) {
            float textWidth = erqal.job.pickerview.FloatMath.ceil(Layout.getDesiredWidth("0",
                    itemsPaint));
        ...
        if (label != null && label.length() > 0) {
            labelWidth = (int) erqal.job.pickerview.FloatMath.ceil(Layout.getDesiredWidth(label,
                    valuePaint));
        }
